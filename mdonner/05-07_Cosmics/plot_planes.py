...
with open(txt_file) as file:
    ...
    event_number = str(re.findall(r'\b\d+\b', data[int(sys.argv[2])-2])[0])   # adapted for a quick checkup; cat 'file' | grep -n 'pALPIDE_' ; this outputs txt line number; go back 4 lines once an event line is identified to get event number
    event = eventData(data,event_number)
    pprint (event)
    print(data[int(sys.argv[2])-4])
    for i in range(0,7):
        print(i)
