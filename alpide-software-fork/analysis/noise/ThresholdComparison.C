...
// compare hists
//---------------------------------------------------------------
Bool_t plot_hists(vector<TH1F*> v_hists, Float_t x_min, Float_t x_max,  TString y_title, TString x_title, TString legend_title, TString legend_subtitle, vector<TString> v_legend_str, TString chip_name, TString plot_name, Bool_t set_color) {
    ...
    // find max bin content of all histograms
    Float_t y_max=0;
    for (UInt_t i=0; i<v_hists.size(); i++) {
        //v_hists[i]->Rebin(2);
        if (v_hists[i]->GetBinContent(v_hists[i]->GetMaximumBin())>y_max) {
            y_max = v_hists[i]->GetBinContent(v_hists[i]->GetMaximumBin());
        }
    }
    y_max*=1.2;
    ...
    for (UInt_t i=0; i<v_hists.size(); i++) {
        ...
        v_hists[i]->GetXaxis()->SetRangeUser(x_min, x_max);
...
//---------------------------------------------------------------
Bool_t compare_vbb(TString path_data, TString run_list_name, TString chip_name, Int_t temperature, Float_t irrad_level, Int_t ithr, Int_t sec) {
    ...
    for (Int_t i_meas=0; i_meas<n_meas; i_meas++) { 
        ...
        for (Int_t i_vbb=0; i_vbb<=6; i_vbb++) {
            ...
            if (is_setting_available(path_results, chip_name, temperature, irrad_level, vbb, vcasn, ithr)) {
                cout << "found combination of settings" << endl;
                hist_name = TString::Format("h_thresholds_TEMP%i.0_VBB%2.1f_VCASN%i_ITHR%i_sec%i",
                  temperature, vbb, vcasn, ithr, sec);
    ...
    TString plot_name = TString::Format("%s/comparisons_threshold/%s_vsVBB_TEMP%i.0_ITHR%i_sec%i.pdf", 
      path_data.Data(), chip_name.Data(), temperature, ithr, sec);

    plot_hists(v_hists, 0, 350, "counts", "threshold [e^{-}]", legend_title, legend_subtitle, v_legend_str, chip_name.Data(), plot_name);
...
//---------------------------------------------------------------
Bool_t compare_temperature(TString path_data, TString run_list_name, TString chip_name, Float_t irrad_level, Float_t vbb, Int_t vcasn, Int_t ithr, Int_t sec) {
    // list of measurements
    vector<TString> run_list;
    ...
    for (Int_t i_meas=0; i_meas<n_meas; i_meas++) { 
        ...
        for (Int_t i_temp=0; i_temp<=40; i_temp++) {
            ...
            if (is_setting_available(path_results, chip_name, temp, irrad_level, vbb, vcasn, ithr)) {
                cout << "found combination of settings" << endl;
                hist_name = TString::Format("h_thresholds_TEMP%i.0_VBB%2.1f_VCASN%i_ITHR%i_sec%i",
                  temp, vbb, vcasn, ithr, sec);
                h = (TH1F*)f_in->Get(hist_name);
                if (h) {
                    v_hists.push_back(h);
    ...
    TString plot_name = TString::Format("%s/comparisons_threshold/%s_vsTemp_VBB%2.1f_VCASN%i_ITHR%i_sec%i.pdf", 
...
// compare settings
//---------------------------------------------------------------
Bool_t compare_settings(TString path_data, TString run_list_name, TString chip_name, Int_t temperature, Float_t irrad_level, TString param_to_compare, Int_t param1, Int_t param2, Int_t sec) {
    ...
    if (param_to_compare.CompareTo("ITHR")==0) {
        ...
        TString plot_name = TString::Format("%s/comparisons_threshold/%s_vsITHR_TEMP%i.0_VBB%2.1f_VCASN%i_sec%i.pdf", 
          path_data.Data(), chip_name.Data(), temperature, vbb, vcasn, sec);

        plot_hists(v_hists, 0, 350, "counts", "threshold [e^{-}]", legend_title, legend_subtitle, v_legend_str, chip_name, plot_name);
...
//---------------------------------------------------------------
Bool_t compare_chips(TString path_data, TString run_list_name, Int_t temperature, Float_t irrad_level, Float_t vbb, Int_t vcasn, Int_t ithr, Int_t sec, TString compare_select) {
    if (!compare_select.EqualTo("thres") && !compare_select.EqualTo("thresnoise")) {
    ...
    else if (irrad_level>0.9e+13) { // real equal condition does not work here.?
        irrad_select = 1;
    } 
    else {
        cout << irrad_level << endl;
        cout << irrad_select << endl;
    ...
    for (Int_t i_meas=0; i_meas<n_meas; i_meas++) { 
        ...
        for (Int_t i_chip=0; i_chip<n_chips; i_chip++) {
            if (is_setting_available(path_results, chip_list[irrad_select][i_chip], temperature, irrad_level, vbb, vcasn, ithr)) {
                cout << "found combination of settings" << endl;
    ...
    //TString plot_name = TString::Format("%s/comparisons_threshold/vsChips_IRRAD%2.1e_TEMP%i.0_VBB%2.1f_VCASN%i_ITHR%i_sec%i.pdf", 
    //  path_data.Data(), irrad_level, temperature, vbb, vcasn, ithr, sec);

    //plot_hists(v_hists, 0, 350, "counts", "threshold [e^{-}]", legend_title, legend_subtitle, v_legend_str, "", plot_name);

    if (compare_select.EqualTo("thres")) { // thres
        TString plot_name = TString::Format("%s/comparisons_threshold/vsChips_thres_IRRAD%2.1e_TEMP%i.0_VBB%2.1f_VCASN%i_ITHR%i_sec%i.pdf", 
          path_data.Data(), irrad_level, temperature, vbb, vcasn, ithr, sec);
        plot_hists(v_hists, 0, 350, "counts", "threshold [/e^{-}]", legend_title, legend_subtitle, v_legend_str, "", plot_name);
    }
    else { // thresnoise
        TString plot_name = TString::Format("%s/comparisons_threshold/vsChips_thresnoise_IRRAD%2.1e_TEMP%i.0_VBB%2.1f_VCASN%i_ITHR%i_sec%i.pdf", 
          path_data.Data(), irrad_level, temperature, vbb, vcasn, ithr, sec);
        plot_hists(v_hists, 0, 20, "counts", "threshold noise [/e^{-}]", legend_title, legend_subtitle, v_legend_str, "", plot_name);
    }
...
//---------------------------------------------------------------
Bool_t compare_irradiation(TString path_data, TString run_list_name, Int_t temperature, Float_t vbb, Int_t vcasn, Int_t ithr, Int_t sec) {
    // list of measured chips
    ...
    for (Int_t i_meas=0; i_meas<n_meas; i_meas++) { 
        ...
        for (Int_t i_irrad=0; i_irrad<n_irrad; i_irrad++) {
            for (Int_t i_chip=0; i_chip<n_chips; i_chip++) {
                if (is_setting_available(path_results, chip_list[i_irrad][i_chip], temperature, irrad_list[i_irrad], vbb, vcasn, ithr)) {
                    cout << "found combination of settings" << endl;
                    hist_name = TString::Format("h_thresholds_TEMP%i.0_VBB%2.1f_VCASN%i_ITHR%i_sec%i",
                      temperature, vbb, vcasn, ithr, sec);
                    h = (TH1F*)f_in->Get(hist_name);
    ...
    TString plot_name = TString::Format("%s/comparisons_threshold/vsIrrad_TEMP%i.0_VBB%2.1f_VCASN%i_ITHR%i_sec%i.pdf", 
      path_data.Data(), temperature, vbb, vcasn, ithr, sec);

    plot_hists(v_hists, 0, 350, "counts", "threshold [e^{-}]", legend_title, legend_subtitle, v_legend_str, "", plot_name, kFALSE);
