...
Bool_t plot_graphs(vector<TGraph*> v_graphs, Float_t y_min, Float_t y_max,  TString y_title, TString x_title, TString legend_title, TString legend_subtitle, vector<TString> v_legend_str, TString chip_name, TString plot_name, Bool_t set_color) {
    ...
    for (UInt_t i=0; i<v_graphs.size(); i++) {
        ...
        v_graphs[i]->SetMarkerSize(i<3?1.5:1.9); // avoid yellow
...
//---------------------------------------------------------------
Bool_t create_noise_graphs_vsITHR(TString path_data, TString run_list_name) {
    ...
    TFile *f_out = new TFile(path_data + "NoiseGraphsvsITHR.root", "RECREATE");

    vector<Float_t> x_vals;
    vector<Float_t> y_vals;
    vector<Float_t> y_err_l;
    vector<Float_t> y_err_h;
                            ...
                            for (Int_t i_sec=0; i_sec<4; i_sec++) {
                                ...
                                for (UInt_t i_ithr=0; i_ithr<ithr_list.size(); i_ithr++) {
                                    ...
                                    hist_name = TString::Format("h_noiseocc_TEMP%i.0_VBB%2.1f_VCASN%i_ITHR%i_RATE%i_BUSY%i_sec%i", 
                                      temp_list[i_temp], vbb_list[i_vbb], vcasn_list[i_vcasn], ithr_list[i_ithr], rate_list[i_rate], busy_list[i_rate], i_sec);
                                    h = (TH1F*)f_in->Get(hist_name);
                                    if (h) {
                                        x_vals.push_back(ithr_list[i_ithr]);
                                        y_vals.push_back(h->GetBinContent(21));
                                        y_err_h.push_back(h->GetBinContent(1)-h->GetBinContent(21));
                                        y_err_l.push_back(h->GetBinContent(21)-h->GetBinContent(100));
                                    }
                                } // ithr

                                TGraphAsymmErrors *g = new TGraphAsymmErrors(x_vals.size(), &x_vals[0], &y_vals[0], 0, 0, &y_err_l[0], &y_err_h[0]);
                                graph_name = TString::Format("g_noiseVsITHR_%s_TEMP%i.0_VBB%2.1f_VCASN%i_RATE%i_BUSY%i_sec%i", 
                                      chip_name.Data(), temp_list[i_temp], vbb_list[i_vbb], vcasn_list[i_vcasn], rate_list[i_rate], busy_list[i_rate], i_sec);
...
// create noise vs ithr graphs for different settings or conditions
//---------------------------------------------------------------
Bool_t create_noise_graphs_vsTemp(TString path_data, TString run_list_name) {
    // list of measurements
    vector<TString> run_list;
    ...
    TString path_results;
    TFile *f_in = new TFile();
    TFile *f_out = new TFile(path_data + "NoiseGraphsvsTemp.root", "RECREATE");
    ...
    for (Int_t i_meas=0; i_meas<n_meas; i_meas++) { 
        ...
        // rate file
        const Int_t n_rate = 1;
        Int_t rate_list[n_rate] = {10000};
        // busy file
        const Int_t n_busy = 1;
        Int_t busy_list[n_rate] = {50};
        // temp file
                    ...
                    for (Int_t i_busy=0; i_busy<n_busy; i_busy++) {
                        for (Int_t i_sec=0; i_sec<4; i_sec++) {
                            ...
                            for (Int_t i_temp=0; i_temp<n_temp; i_temp++) {
                                ...
                                //cout << " ithr: " << ithr_list[i_ithr] << endl;
                                hist_name = TString::Format("h_noiseocc_TEMP%i.0_VBB%2.1f_VCASN%i_ITHR%i_RATE%i_BUSY%i_sec%i", 
                                  temp_list[i_temp], vbb_list[i_vbb], vcasn_list[i_vbb], ithr_list[i_ithr], rate_list[i_rate], busy_list[i_rate], i_sec);
                                h = (TH1F*)f_in->Get(hist_name);
                                if (h) {
                                    //x_vals.push_back((temp_list[i_temp]+i_ithr*0.05));
                                    x_vals.push_back(temp_list[i_temp]);
                                    y_vals.push_back(h->GetBinContent(21));
                                    y_err_h.push_back(h->GetBinContent(1)-h->GetBinContent(21));
                                    y_err_l.push_back(h->GetBinContent(21)-h->GetBinContent(100));
                                }
                            ...
                            graph_name = TString::Format("g_noiseVsTemp_%s_VBB%2.1f_VCASN%i_ITHR%i_RATE%i_BUSY%i_sec%i", 
                                  chip_name.Data(), vbb_list[i_vbb], vcasn_list[i_vbb], ithr_list[i_ithr], rate_list[i_rate], busy_list[i_rate], i_sec);
                            g->SetName(graph_name);
                            f_out->cd();
                            g->Write();
                        }
...
// vsITHR graphs: compare different irradiation levels
//---------------------------------------------------------------
Bool_t compare_vsITHR_Irrad(TString path_data, Int_t temperature, Float_t vbb, Int_t vcasn, Int_t rate, Int_t busy, Int_t sec) {
    ...
    vector<TString> v_legend_str;
    TString legend_title = TString::Format("VBB-%2.1f_VCASN%i_sec%i", vbb, vcasn, sec);
    TString legend_subtitle = TString::Format("TEMP %i.0 #circC", temperature);
    //TString legend_subtitle = TString::Format("RATE %i Hz, BUSY %i #mus", rate, busy);
    //TString legend_subtitle = TString::Format("");
    TGraph *g;
    TString graph_name;
    ...
    for (Int_t i_irrad=0; i_irrad<n_irrad; i_irrad++) {
        for (Int_t i_chip=0; i_chip<n_chips; i_chip++) {
            ...
            graph_name = TString::Format("g_noiseVsITHR_%s_TEMP%i.0_VBB%2.1f_VCASN%i_RATE%i_BUSY%i_sec%i", 
              chip_list[i_irrad][i_chip].Data(), temperature, vbb, vcasn, rate, busy, sec);
    ...
    TString plot_name = TString::Format("%s/comparisons_noise/summary_plots/vsIrrad_TEMP%i.0_VBB%2.1f_VCASN%i_sec%i.pdf", 
      path_data.Data(), temperature, vbb, vcasn, sec);

    plot_graphs(v_graphs, 1e-9, 1e-3, "noise occupancy [/event/pixel]", "ITHR [DAC]", legend_title, legend_subtitle, v_legend_str, "", plot_name, kFALSE);
...
//---------------------------------------------------------------
Bool_t compare_vsTemp_Irrad(TString path_data, Float_t vbb, Int_t vcasn, Int_t ithr, Int_t rate, Int_t busy, Int_t sec) {
    ...
    for (Int_t i_irrad=0; i_irrad<n_irrad; i_irrad++) {
        for (Int_t i_chip=0; i_chip<n_chips; i_chip++) {
            ...
            graph_name = TString::Format("g_noiseVsTemp_%s_VBB%2.1f_VCASN%i_ITHR%i_RATE%i_BUSY%i_sec%i", 
              chip_list[i_irrad][i_chip].Data(), vbb, vcasn, ithr, rate, busy, sec);
    ...
    TString plot_name = TString::Format("%s/comparisons_noise/summary_plots/vsIrrad_VBB%2.1f_VCASN%i_ITHR%i_sec%i.pdf", 
      path_data.Data(), vbb, vcasn, ithr, sec);

    plot_graphs(v_graphs, 1e-9, 1e-3, "noise occupancy [/event/pixel]", "temperature [#circC]", legend_title, legend_subtitle, v_legend_str, "", plot_name, kFALSE);
