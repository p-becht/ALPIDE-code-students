void PlotChipsPixelThreshold(const TString sDataset="dataset.txt")
{
  if (gSystem->Load("$PA_ROOT/lib/libpAAnalysis")<0) {
    ::Error("PlotChipsPixelThreshold.C::PlotChipsPixelThreshold", "Can not load lib $PA_ROOT/lib/libpAAnalysis");
    return;
  }
//=============================================================================

  if (gSystem->AccessPathName(sDataset.Data())) {
    ::Error("PlotChipsPixelThreshold.C::PlotChipsPixelThreshold", "Dataset %s does not exist!", sDataset.Data());
    return;
  }
//=============================================================================
...
//=============================================================================
  ...
  while (!dataList.eof()) {
    ...
    ::Info("PlotChipsPixelThreshold.C::PlotChipsPixelThreshold", "Chip ID%d: %s", nc, sDataFile.Data());
    sID = CheckChipIdentifier(sDataFile, sFormat);

    for (int i=0; i<=pa->GetNScans(); i++) {
      if (pa->GetScanType(i)!=TpAUtils::SCANTYPE_THRESHOLD) continue;
      TpAThresholdScan *pScan = (TpAThresholdScan*)pa->RetrieveScan(i); if (!pScan) continue;
  ...
  if (ns<=0) return;
  ::Info("PlotChipsPixelThreshold.C::PlotChipsPixelThreshold", "%d files have been analyzed", nc);
//=============================================================================
...
//=============================================================================
  ...
  can = MakeCanvas("ChipsPixelThresholdScanDead");
  ...
  tex->DrawLatex(0.1, 0.92, "Threshold scan, threshold = 0");
  CanvasEnd(can);
//=============================================================================

  can = MakeCanvas("ChipsPixelThresholdScanInefficient");
  ...
  tex->SetNDC();
  tex->SetTextSizePixels(24);
  tex->DrawLatex(0.1, 0.92, "Threshold scan, threshold = 0 && #chi^{2}>4");
  CanvasEnd(can);
//=============================================================================

  can = MakeCanvas("ChipsPixelThresholdScanNoisy");
  can->SetRightMargin(0.12);
  can->SetBottomMargin(0.2);
  can->SetGridx();
  can->SetGridy();
  ...
  tex->SetNDC();
  tex->SetTextSizePixels(24);
  tex->DrawLatex(0.1, 0.92, "Threshold scan, #chi^{2}>4");
  CanvasEnd(can);
//=============================================================================

  cout << endl;
  ::Info("PlotChipsPixelThreshold.C::PlotChipsPixelThreshold", "Markdown table");
  ...
  ::Info("PlotChipsPixelThreshold.C::PlotChipsPixelThreshold", "End of markdonw table");
  cout << endl;
//=============================================================================

  ::Info("PlotChipsPixelThreshold.C::PlotChipsPixelThreshold", "DONE");
  return;
}
