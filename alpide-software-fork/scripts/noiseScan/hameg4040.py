...
def powerDownChannel(sour, ch):
    sour.write("INST OUT%i\n" % (int(ch)))
    sour.write("OUTP OFF\n")
...
def measureCurr(sour):
    ...
    print "%0.4fA\t%0.4fA\t%0.4fA" % ( val[0], val[1], val[2], val[3])
...
def main():
    ...
    # switch mode
    if mode==0:
        # prepare the voltage source, set current limits
        init2030(sour, (float(sys.argv[3]), float(sys.argv[4]), float(sys.argv[5]), float(sys.argv[6])))
    elif mode==1:
        # set a voltage value to a channel
        sour.write("INST OUT%d\n" % int(sys.argv[3]))
        sour.write("SOUR:VOLT %f\n" % float(sys.argv[4]))
    elif mode==2: 
        # ramp channel 1, 2, 3 and 4 from 0 to v
        ramp(sour, (float(sys.argv[3]), float(sys.argv[4]), float(sys.argv[5]), float(sys.argv[6])), int(sys.argv[7]))
    elif mode==3:
        # ramp channel 2 from v1 to v2 
        rampFromTo(sour, (float(sys.argv[3]), float(sys.argv[4])), int(sys.argv[5]))
    elif mode==4:
        # measure current and check whether it is in range
        return trip(sour)
    elif mode==5:
        # measure and print currents for all channels
        measureCurr(sour)
    elif mode==6:
        # measure and print currents for all channels
        powerDownChannel(sour, (int(sys.argv[3])))
    else:
        # power down complete supply 
        powerDown(sour)
